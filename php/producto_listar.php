...
$campos = "producto.producto_id,producto.producto_codigo,producto.producto_nombre,producto.producto_precio,producto.producto_stock,
producto.producto_foto,categoria.categoria_nombre,usuario.usuario_nombre";
...
$Npaginas = ceil($total / $registros); // redondeo de número de páginas

if ($total >= 1 && $Npaginas >= $pagina) {  // hay registros y estamos en una página válida
    $contador = $inicio + 1;
    $pag_inicio = $inicio + 1;
        ...
        $tabla .= '
                    <article class="media">
                        <figure class="media-left">
                            <p class="image is-64x64">'; // CORREGIDO: cerramos la etiqueta de <p>
                                
        // Verificar si el archivo de imagen existe
        if (is_file("./img/producto/" . $rows['producto_foto'])) {
            $tabla .= '<img src="./img/producto/' . $rows['producto_foto'] . '">';
        } else {
            $tabla .= '<img src="./img/defecto.png">'; // Imagen por defecto si no existe
        }
        ...
        $tabla .= '     </p>
                        ...
                        <div class="media-content">
                            ...
                            <div class="has-text-right">
                                <a href="index.php?vista=producto_imagen&producto_id_up=' . $rows['producto_id'] . '" class="button is-link is-rounded is-small">Imagen</a>
                                <a href="index.php?vista=producto_actualizar&producto_id_up=' . $rows['producto_id'] . '" class="button is-success is-rounded is-small">Actualizar</a>
...
if ($total >= 1 && $pagina <= $Npaginas) {
    $tabla .= '<p class="has-text-right">Mostrando productos <strong>' . $pag_inicio . '</strong> al <strong>' . $pag_fin . '</strong> de un <strong>total de ' . $total . '</strong></p>';
}
